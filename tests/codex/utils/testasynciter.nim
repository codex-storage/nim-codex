...
asyncchecksuite "Test AsyncIter":
  ...
  test "using with async generator":
    ...
    let iter = AsyncIter[int].new(asyncGen, () => intIter.finished)
    ...
    check nextRes.error.msg == "AsyncIter is finished but next item was requested"

  test "getting async iter for simple sync range iterator":
    let iter1 = AsyncIter[int].new(0 ..< 5)
  ...
  test "Should map each item using `map`":
    let iter1 = AsyncIter[int].new(0 ..< 5).delayBy(10.millis)
  ...
  test "Should propagate cancellation error immediately":
    ...
    let fut: Future[Option[?!string]].Raising([CancelledError]) =
      Future[Option[?!string]].Raising([CancelledError]).init("testasynciter")

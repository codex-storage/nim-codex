...
type NBytes* = distinct Natural

template basicMaths(T: untyped) =
  proc `+`*(x: T, y: static[int]): T =
    T(`+`(x.Natural, y.Natural))
...
template divMaths(T: untyped) =
  proc `mod`*(x, y: T): T =
    T(`mod`(x.Natural, y.Natural))

  proc `div`*(x, y: T): Natural =
    `div`(x.Natural, y.Natural)
...
proc `$`*(ts: NBytes): string =
  $(int(ts)) & "'NByte"

proc `'nb`*(n: string): NBytes =
  parseInt(n).NBytes

logutils.formatIt(NBytes):
  $it
...
proc KiBs*(v: Natural): NBytes =
  v.NBytes * KiB

proc MiBs*(v: Natural): NBytes =
  v.NBytes * MiB

proc GiBs*(v: Natural): NBytes =
  v.NBytes * GiB

...
type
  CacheStore* = ref object of BlockStore
    ...
    cidAndProofCache: LruCache[(Cid, Natural), (Cid, CodexMerkleProof)]
...
proc getCidAndProof(self: CacheStore, treeCid: Cid, index: Natural): ?!(Cid, CodexMerkleProof) =
...
method getBlockAndProof*(self: CacheStore, treeCid: Cid, index: Natural): Future[?!(Block, CodexMerkleProof)] {.async.} =
  without cidAndProof =? self.getCidAndProof(treeCid, index), err:
    return failure(err)
...
proc new*(
  ...
  let
    ...
    cidAndProofCache = newLruCache[(Cid, Natural), (Cid, CodexMerkleProof)](size)
    store = CacheStore(
      cache: cache,

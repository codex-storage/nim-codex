...
proc encode*(manifest: Manifest): ?!seq[byte] =
  ...
  #     bytes slotsRoot = 1;            # Decimal encoded field-element
  ...
  if manifest.protected:
    ...
    if manifest.verifiable:
      ...
      verificationInfo.write(1, manifest.slotsRoot.data.buffer)
...
proc decode*(_: type Manifest, data: openArray[byte]): ?!Manifest =
  ...
  if protected:
    ...
    if verifiable:
      if pbVerificationInfo.getField(1, slotsRoot).isErr:
        return failure("Unable to decode `slotsRoot` from manifest!")
  ...
  if verifiable:
    let
      verificationRootCid = ? Cid.init(slotsRoot).mapFailure
      slotRootCids = slotRoots.mapIt(? Cid.init(it).mapFailure)

    return Manifest.new(
      manifest = self,
      slotsRoot = verificationRootCid,

...
proc `default`*(presets: openArray[NetworkPreset]): NetworkPreset = presets[0]
...
proc find*(presets: openArray[NetworkPreset], p: string): Result[NetworkPreset, string] =
  for preset in presets:
    if preset.name == p:
      return ok(preset)
